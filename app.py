...
SPREADSHEET_ID = "1dD_lYX6pOAfioM-esf4G3caNfTzfAbd4"
...
def load_data():
    ...
    sprint_best = sprint.groupby("DPL ID").agg(sprint_10=("10yd","min"),sprint_20=("20yd","min"),sprint_30=("30yd","min"),sprint_name=("Full Name Reverse","first"),sprint_year=("Year","first")).reset_index()
    fp_best = fp.groupby("DPL ID").agg(concentric_impulse=("Concentric Impulse [Ns]","max"),mrsi=("RSI-Modified [m/s]","max"),peak_power=("Peak Power [W]","max"),rel_peak_power=("Peak Power / BM [W/kg]","max"),fp_name=("Full Name Reverse","first"),fp_position=("Position","first"),fp_year=("Year","first")).reset_index()
    anthro_best = anthro.groupby("DPL ID").agg(height_cm=("height_cm","first"),weight_kg=("weight_kg","first"),wingspan_cm=("wingspan_cm","first"),anthro_name=("Full Name Reverse","first"),anthro_position=("Position","first"),anthro_year=("Year","first")).reset_index()
...
def compute_scores(df):
    ...
    for _, r in df.iterrows():
        ...
        pm=[(h_p,0.20),(w_p,0.15),(mr_p,0.20),(rp_p,0.20),(s30_p,0.15),(ws_p,0.10)] if is_p else [(h_p,0.20),(w_p,0.20),(mr_p,0.25),(rp_p,0.20),(s30_p,0.15)]
        pp,pw=[],[]
        for v,wt in pm:
            if not pd.isna(v): pp.append(v*wt);pw.append(wt)
        pot=(sum(pp)/sum(pw)) if pw else np.nan
        rows.append({"athletic_score":ath,"potential_score":pot,"ci_pct":ci_p,"mrsi_pct":mr_p,"s30_pct":s30_p,"h_pct":h_p,"w_pct":w_p,"rpp_pct":rp_p,"ws_pct":ws_p})
...
def gauge(score, title, color):
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={"axis":{"range":[0,100],"tickfont":{"color":"#8b949e","size":10}},"bar":{"color":color,"thickness":0.25},"bgcolor":"#161b22","borderwidth":0,
               "steps":[{"range":[0,40],"color":"#1a1f2e"},{"range":[40,60],"color":"#1a2030"},{"range":[60,80],"color":"#1a2820"},{"range":[80,100],"color":"#1a3020"}],
               "threshold":{"line":{"color":color,"width":3},"thickness":0.8,"value":sd}}
    ))
    fig.update_layout(height=200,margin=dict(l=20,r=20,t=40,b=10),paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",font={"family":"Inter"})
    return fig

def radar(labels, values):
    vals=[v if not pd.isna(v) else 0 for v in values]
    fig=go.Figure(go.Scatterpolar(r=vals+[vals[0]],theta=labels+[labels[0]],fill="toself",fillcolor="rgba(56,139,253,0.15)",line=dict(color="#388bfd",width=2),marker=dict(color="#388bfd",size=6)))
    fig.update_layout(polar=dict(bgcolor="#161b22",radialaxis=dict(visible=True,range=[0,100],tickfont=dict(color="#8b949e",size=9),gridcolor="#30363d",linecolor="#30363d"),angularaxis=dict(tickfont=dict(color="#e6edf3",size=11),gridcolor="#30363d",linecolor="#30363d")),
                      showlegend=False,paper_bgcolor="rgba(0,0,0,0)",margin=dict(l=50,r=50,t=30,b=30),height=300,font={"family":"Inter"})
    return fig
...
filtered = df[df["year"].isin(sel_years)&df["position"].isin(sel_positions)&(df["athletic_score"].fillna(0)>=min_ath)&(df["potential_score"].fillna(0)>=min_pot)].copy()
sc,sa={"Athletic Score":("athletic_score",False),"Potential Score":("potential_score",False),"30yd Sprint":("sprint_30",True),"mRSI":("mrsi",False),"Concentric Impulse":("concentric_impulse",False)}[sort_by]
...
with tab1:
    disp=filtered[["name","position","year","athletic_score","potential_score","concentric_impulse","mrsi","sprint_10","sprint_20","sprint_30","height_in","weight_lb","rel_peak_power","wingspan_in"]].copy()
    disp.rename(columns={"name":"Athlete","position":"Position","year":"Year","athletic_score":"Athletic Score","potential_score":"Potential Score","concentric_impulse":"Conc. Impulse (Ns)","mrsi":"mRSI (m/s)","sprint_10":"10yd (s)","sprint_20":"20yd (s)","sprint_30":"30yd (s)","height_in":"Height (in)","weight_lb":"Weight (lb)","rel_peak_power":"Rel. Peak Power (W/kg)","wingspan_in":"Wingspan (in)"},inplace=True)
    for c in ["Athletic Score","Potential Score","Conc. Impulse (Ns)","mRSI (m/s)","10yd (s)","20yd (s)","30yd (s)","Height (in)","Weight (lb)","Rel. Peak Power (W/kg)","Wingspan (in)"]:
        if c in disp.columns: disp[c]=pd.to_numeric(disp[c],errors="coerce").round(2)
    st.dataframe(disp.reset_index(drop=True),use_container_width=True,height=500,
        column_config={"Athletic Score":st.column_config.ProgressColumn("Athletic Score",min_value=0,max_value=100,format="%.1f"),
...
with tab2:
    ...
    if not player_names:
        ...
        with c2:
            if is_p: st.markdown("<div style='text-align:right;color:#388bfd;font-size:12px;font-weight:600;padding-top:8px'>⚾ PITCHER — WINGSPAN INCLUDED</div>",unsafe_allow_html=True)
